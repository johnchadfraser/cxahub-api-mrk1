...
const tableName = 'cxahub.t_notification';
const viewName = 'cxahub.v_notification';
const orderBy = 'ORDER BY sort_id ASC';

//Set services used by route.
const NotificationService = require('../../../services/utils/NotificationService');
...
router.get('/', [auth, service], cache(cachMins), async (req, res) => {
    ...
    try {
      ...
      if(req.query.nt_name) {
  
        qry = qry + ` AND nt_name = '${req.query.nt_name}'`;
...
router.post('/', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { nt_name, nt_description, user_id, sort_id, status_id } = req.body

    //Check if record exists.
    const checkExist = await NotificationService.findByName(nt_name);

    //If record exists, do not post.
    if(checkExist != '') {

      res.send(`Member already exists: ${nt_name}`)

    } else {

      const { rows } = await db.query(`INSERT INTO ${tableName} (nt_name, nt_description, user_id, sort_id, status_id) VALUES ($1, $2, $3, $4, $5) RETURNING *`, [nt_name, nt_description, user_id, sort_id, status_id ])
...
router.put('/:id', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params
    const { nt_name, nt_description, user_id, sort_id, status_id  } = req.body
    const { rows } = await db.query(`UPDATE ${tableName} SET nt_name = $2, nt_description = $3, nt_time_updated = now(), user_id = $4, sort_id = $5, status_id = $6 WHERE id = $1`, [id, nt_name, nt_description, user_id, sort_id, status_id])
