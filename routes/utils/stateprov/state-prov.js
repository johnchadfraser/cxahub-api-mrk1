...
const tableName = 'cxahub.t_state_prov';
const viewName = 'cxahub.v_state_prov';
const orderBy = 'ORDER BY sort_id ASC';

//Set services used by route.
const StateProvService = require('../../../services/utils/StateProvService');
...
router.get('/', [auth, service], cache(cachMins), async (req, res) => {
    ...
    try {
      ...
      if(req.query.sp_name) {
  
        qry = qry + ` AND sp_name = '${req.query.sp_name}'`;
...
router.post('/', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { sp_name, sp_abbreviation, cntry_id, user_id, sort_id, status_id } = req.body

    //Check if record exists.
    const checkExist = await StateProvService.findByName(sp_name);

    //If record exists, do not post.
    if(checkExist != '') {

      res.send(`Member already exists: ${sp_name}`)

    } else {

      const { rows } = await db.query(`INSERT INTO ${tableName} (sp_name, sp_abbreviation, cntry_id, user_id, sort_id, status_id) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`, [sp_name, sp_abbreviation, cntry_id, user_id, sort_id, status_id])
...
router.put('/:id', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params
    const { sp_name, sp_abbreviation, cntry_id, user_id, sort_id, status_id } = req.body
    const { rows } = await db.query(`UPDATE ${tableName} SET sp_name = $2, sp_abbreviation = $3, sp_time_updated = now(), cntry_id = $4, user_id = $5, sort_id = $6, status_id = $7 WHERE id = $1`, [id, sp_name, sp_abbreviation, cntry_id, user_id, sort_id, status_id])
