...
const tableName = 'cxahub.t_time';
const viewName = 'cxahub.v_time';
const orderBy = 'ORDER BY sort_id ASC';

//Set services used by route.
const TimeService = require('../../../services/utils/TimeService');
...
router.get('/', [auth, service], cache(cachMins), async (req, res) => {
    ...
    try {
      ...
      if(req.query.time_name) {
  
        qry = qry + ` AND time_name = '${req.query.time_name}'`;
...
router.post('/', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { time_name, time_value, user_id, sort_id, status_id } = req.body

    //Check if record exists.
    const checkExist = await TimeService.findByName(time_name);

    //If record exists, do not post.
    if(checkExist != '') {

      res.send(`Member already exists: ${time_name}`)

    } else {

      const { rows } = await db.query(`INSERT INTO ${tableName} (time_name, time_value, user_id, sort_id, status_id) VALUES ($1, $2, $3, $4, $5) RETURNING *`, [time_name, time_value, user_id, sort_id, status_id ])
...
router.put('/:id', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params
    const { time_name, time_value, user_id, sort_id, status_id } = req.body
    const { rows } = await db.query(`UPDATE ${tableName} SET time_name = $2, time_value = $3, time_time_updated = now(), user_id = $4, sort_id = $5, status_id = $6 WHERE id = $1`, [id, time_name, time_value, user_id, sort_id, status_id])
