...
const tableName = 'cxahub.t_mobile_carrier';
const viewName = 'cxahub.v_mobile_carrier';
const orderBy = 'ORDER BY sort_id ASC';

//Set services used by route.
const MobileCarrierService = require('../../../services/utils/MobileCarrierService');
...
router.get('/', [auth, service], cache(cachMins), async (req, res) => {
    ...
    try {
      ...
      if(req.query.mc_name) {
  
        qry = qry + ` AND mc_name = '${req.query.mc_name}'`;
...
router.post('/', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { mc_name, mc_email, user_id, sort_id, status_id } = req.body

    //Check if record exists.
    const checkExist = await MobileCarrierService.findByName(mc_name);

    //If record exists, do not post.
    if(checkExist != '') {

      res.send(`Member already exists: ${mc_name}`)

    } else {

      const { rows } = await db.query(`INSERT INTO ${tableName} (mc_name, mc_email, user_id, sort_id, status_id) VALUES ($1, $2, $3, $4, $5) RETURNING *`, [mc_name, mc_email, user_id, sort_id, status_id])
...
router.put('/:id', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params
    const { mc_name, mc_email, user_id, sort_id, status_id} = req.body
    const { rows } = await db.query(`UPDATE ${tableName} SET mc_name = $2, mc_email = $3, mc_time_updated = now(), user_id = $4, sort_id = $5, status_id = $6 WHERE id = $1`, [id, mc_name, mc_email, user_id, sort_id, status_id])
