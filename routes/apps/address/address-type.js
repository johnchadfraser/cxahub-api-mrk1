...
const tableName = "cxahub.t_address_type";
const viewName = "cxahub.v_address_type";
const orderBy = "ORDER BY sort_id ASC";

//Set services used by route.
const AddressTypeService = require("../../../services/address/AddressTypeService");
...
router.get("/", [auth, service], cache(cachMins), async (req, res) => {
  try {
    if (req.query.cols) {
      qry = `SELECT ${req.query.cols} FROM ${viewName} WHERE 0=0`;
    } else {
      qry = `SELECT * FROM ${viewName} WHERE 0=0`;
    }

    if (req.query.at_name) {
      qry = qry + ` AND at_name = '${req.query.at_name}'`;
    }
...
router.post("/", [auth, service], async (req, res) => {
  try {
    const { at_name, user_id, sort_id, status_id } = req.body;

    //Check if record exists.
    const checkExist = await AddressTypeService.findByName(at_name);

    //If record exists, do not post.
    if (checkExist != "") {
      res.send(`Member already exists: ${at_name}`);
    } else {
      const { rows } = await db.query(
        `INSERT INTO ${tableName} (at_name, user_id, sort_id, status_id) VALUES ($1, $2, $3, $4) RETURNING *`,
        [at_name, user_id, sort_id, status_id]
      );
...
router.put("/:id", [auth, service], async (req, res) => {
  try {
    ...
    const { at_name, user_id, sort_id, status_id } = req.body;
    const { rows } = await db.query(
      `UPDATE ${tableName} SET at_name = $2, at_time_updated = now(), user_id = $3, sort_id = $4, status_id = $5 WHERE id = $1`,
      [id, at_name, user_id, sort_id, status_id]
    );
