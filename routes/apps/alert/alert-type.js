...
const tableName = 'cxahub.t_alert_type';
const viewName = 'cxahub.v_alert_type';
const orderBy = 'ORDER BY sort_id ASC';

//Set services used by route.
const AlertTypeService = require('../../../services/alert/AlertTypeService');
...
router.post('/', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const checkExist = await AlertTypeService.findByName(at_name);
    ...
    if(checkExist != '') {
      ...
      const { rows } = await db.query(`INSERT INTO ${tableName} (at_name, user_id, sort_id, status_id) VALUES ($1, $2, $3, $4) RETURNING *`, [at_name, user_id, sort_id, status_id])
...
router.put('/:id', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params
    const { at_name, user_id, sort_id, status_id } = req.body
    const { rows } = await db.query(`UPDATE ${tableName} SET at_name = $2,at_time_updated = now(), user_id = $3, sort_id = $4, status_id = $5 WHERE id = $1`, [id, at_name, user_id, sort_id, status_id])
