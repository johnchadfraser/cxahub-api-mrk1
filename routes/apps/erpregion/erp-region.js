...
const tableName = 'cxahub.t_erp_region';
const viewName = 'cxahub.v_erp_region';
const orderBy = 'ORDER BY sort_id ASC';

//Set services used by route.
const ERPRegionService = require('../../../services/erpregion/ERPRegionService');
...
router.get('/', [auth, service], cache(cachMins), async (req, res) => {
    ...
    try {
      ...
      if(req.query.erpr_name) {
  
        qry = qry + ` AND erpr_name = '${req.query.erpr_name}'`;
...
router.post('/', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { erpr_name, user_id, sort_id, status_id } = req.body

    //Check if record exists.
    const checkExist = await ERPRegionService.findByName(erpr_name);

    //If record exists, do not post.
    if(checkExist != '') {

      res.send(`Member already exists: ${erpr_name}`)

    } else {

      const { rows } = await db.query(`INSERT INTO ${tableName} (erpr_name, user_id, sort_id, status_id) VALUES ($1, $2, $3, $4) RETURNING *`, [erpr_name, user_id, sort_id, status_id])
...
router.put('/:id', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params
    const { erpr_name, user_id, sort_id, status_id } = req.body
    const { rows } = await db.query(`UPDATE ${tableName} SET erpr_name = $2, erpr_time_updated = now(), user_id = $3, sort_id = $4, status_id = $5 WHERE id = $1`, [id, erpr_name, user_id, sort_id, status_id])
