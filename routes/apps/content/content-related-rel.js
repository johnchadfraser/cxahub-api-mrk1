...
const tableName = "cxahub.t_content_related_rel";
const viewName = "cxahub.v_content_related_rel";
const orderBy = "";

//Set messaging.
const createSuccessMessage =
  "You have created the content related rel. successfully.";
const deleteSuccessMessage =
  "You have deleted the content related rel. successfully.";

//Set services used by route.
//const ContentrelatedRelService = require('../../../services/content/ContentrelatedRelService');
...
router.get("/", [auth, service], cache(cachMins), async (req, res) => {
  try {
    ...
    if (req.query.c_id && req.query.related_c_id) {
      qry =
        qry +
        ` AND (c_id = '${req.query.c_id}' AND related_c_id = '${req.query.related_c_id}')`;
    }
...
router.post(
  ...
  wrap(async function (req, res) {
    const { c_id, related_c_id } = req.body;

    //First delete any existing relationship records.
    await ContentService.deleteContentRelatedRel(c_id);

    await db.query(
      `INSERT INTO ${tableName} (c_id, related_c_id) VALUES ($1, $2) RETURNING *`,
      [c_id, related_c_id]
    );
