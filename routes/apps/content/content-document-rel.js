...
const tableName = 'cxahub.t_content_document_rel';
const viewName = 'cxahub.v_content_document_rel';
const orderBy = '';

//Set messaging.
const createSuccessMessage = 'You have created the content document rel. successfully.';
const deleteSuccessMessage = 'You have deleted the content document rel. successfully.';
...
router.get('/', [auth, service], cache(cachMins), async (req, res) => {
  ...
  try {
    ...
    if(req.query.c_id && req.query.doc_id) {

      qry = qry + ` AND (c_id = '${req.query.c_id}' AND doc_id = '${req.query.doc_id}')`;
    ...
    if(req.query.doc_id) {
      ...
      qry = qry + ` AND doc_id = '${req.query.doc_id}'`;
...
router.post('/', [auth, service], wrap(async function(req, res) {
  ...
  const { c_id, doc_id } = req.body

  //First delete any existing relationship records.
  await ContentService.deleteContentDocumentRel(c_id);

  await db.query(`INSERT INTO ${tableName} (c_id, doc_id) VALUES ($1, $2) RETURNING *`, [c_id, doc_id])
