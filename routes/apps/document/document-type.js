...
const tableName = 'cxahub.t_document_type';
const viewName = 'cxahub.v_document_type';
const orderBy = 'ORDER BY sort_id ASC';

//Set services used by route.
const DocumentTypeService = require('../../../services/document/DocumentTypeService');
...
router.get('/', [auth, service], cache(cachMins), async (req, res) => {
    ...
    try {
      ...
      if(req.query.doct_name) {
  
        qry = qry + ` AND doct_name = '${req.query.doct_name}'`;
...
router.post('/', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { doct_name, doct_path, user_id, sort_id, status_id } = req.body

    //Check if record exists.
    const checkExist = await DocumentTypeService.findByName(doct_name);

    //If record exists, do not post.
    if(checkExist != '') {

      res.send(`Member already exists: ${doct_name}`)

    } else {

      const { rows } = await db.query(`INSERT INTO ${tableName} (doct_name, doct_path, user_id, sort_id, status_id) VALUES ($1, $2, $3, $4, $5) RETURNING *`, [doct_name, doct_path, user_id, sort_id, status_id])
...
router.put('/:id', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params
    const { doct_name, doct_path, user_id, sort_id, status_id } = req.body
    const { rows } = await db.query(`UPDATE ${tableName} SET doct_name = $2, doct_path = $3, doct_time_updated = now(), user_id = $4, sort_id = $5, status_id = $6 WHERE id = $1`, [id, doct_name, doct_path, user_id, sort_id, status_id])
