...
const tableName = 'cxahub.t_brand_industry_rel';
const viewName = 'cxahub.v_brand_industry_rel';
const orderBy = '';

//Set messaging.
const createSuccessMessage = 'You have created the brand industry rel. successfully.';
const deleteSuccessMessage = 'You have deleted the brand industry rel. successfully.';

//Set services used by route.
const BrandIndustryRelService = require('../../../services/brand/BrandIndustryRelService');
...
router.get('/', [auth, service], cache(cachMins), async (req, res) => {
  ...
  try {
    ...
    if(req.query.b_id && req.query.i_id) {

      qry = qry + ` AND (b_id = '${req.query.b_id}' AND i_id = '${req.query.i_id}')`;
...
router.post('/', [auth, service], wrap(async function(req, res) {
  ...
  const { b_id, i_id } = req.body

  //First delete any existing relationship records.
  await BrandIndustryRelService.deleteBrandIndustryRel(b_id);

  await db.query(`INSERT INTO ${tableName} (b_id, i_id) VALUES ($1, $2) RETURNING *`, [b_id, i_id])
