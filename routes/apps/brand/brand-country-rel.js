...
const tableName = 'cxahub.t_brand_country_rel';
const viewName = 'cxahub.v_brand_country_rel';
const orderBy = '';

//Set messaging.
const createSuccessMessage = 'You have created the brand country rel. successfully.';
const deleteSuccessMessage = 'You have deleted the brand country rel. successfully.';

//Set services used by route.
const BrandCountryRelService = require('../../../services/brand/BrandCountryRelService');
...
router.get('/', [auth, service], cache(cachMins), async (req, res) => {
  ...
  try {
    ...
    if(req.query.b_id && req.query.cntry_id) {

      qry = qry + ` AND (b_id = '${req.query.b_id}' AND cntry_id = '${req.query.cntry_id}')`;
...
router.post('/', [auth, service], wrap(async function(req, res) {
  ...
  const { b_id, cntry_id } = req.body

  //First delete any existing relationship records.
  await BrandCountryRelService.deleteBrandCountryRel(b_id);

  await db.query(`INSERT INTO ${tableName} (b_id, cntry_id) VALUES ($1, $2) RETURNING *`, [b_id, cntry_id])
