...
const tableName = 'cxahub.t_brand';
const viewName = 'cxahub.v_brand';
...
router.get('/', [auth, service], cache(cachMins), async (req, res) => {
    ...
    try {
      ...
      if(req.query.b_name) {
        ...
        qry = qry + ` AND b_name = '${req.query.b_name}'`;
...
router.post('/', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { b_name, user_id, sort_id, status_id } = req.body

    //Check if record exists.
    const checkExist = await BrandService.findByName(b_name);

    //If record exists, do not post.
    if(checkExist != '') {

      res.send(`Member already exists: ${b_name}`)

    } else {

      const { rows } = await db.query(`INSERT INTO ${tableName} (b_name, user_id, sort_id, status_id) VALUES ($1, $2, $3, $4) RETURNING *`, [b_name, user_id, sort_id, status_id ])
...
router.put('/:id', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params
    const { b_name, user_id, sort_id, status_id  } = req.body
    const { rows } = await db.query(`UPDATE ${tableName} SET b_name = $2, b_time_updated = now(), user_id = $3, sort_id = $4, status_id = $5 WHERE id = $1`, [id, b_name, user_id, sort_id, status_id])
