...
const tableName = 'cxahub.t_brand_erp_region_rel';
const viewName = 'cxahub.v_brand_erp_region_rel';
const orderBy = '';

//Set messaging.
const createSuccessMessage = 'You have created the brand ERP region rel. successfully.';
const deleteSuccessMessage = 'You have deleted the brand ERP region rel. successfully.';

//Set services used by route.

const BrandERPRegionRelService = require('../../../services/brand/BrandERPRegionRelService');
...
router.get('/', [auth, service], cache(cachMins), async (req, res) => {
  ...
  try {
    ...
    if(req.query.b_id && req.query.erpr_id) {

      qry = qry + ` AND (b_id = '${req.query.b_id}' AND erpr_id = '${req.query.erpr_id}')`;
...
router.post('/', [auth, service], wrap(async function(req, res) {
  ...
  const { b_id, erpr_id } = req.body

  //First delete any existing relationship records.
  await BrandERPRegionRelService.deleteBrandERPRegionRel(b_id);

  await db.query(`INSERT INTO ${tableName} (b_id, erpr_id) VALUES ($1, $2) RETURNING *`, [b_id, erpr_id])
