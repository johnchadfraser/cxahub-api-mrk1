...
const tableName = 'cxahub.t_event_delivery_type';
const viewName = 'cxahub.v_event_delivery_type';
const orderBy = 'ORDER BY sort_id ASC';

//Set services used by route.
const EventDeliveryTypeService = require('../../../services/event/EventDeliveryTypeService');
...
router.get('/', [auth, service], cache(cachMins), async (req, res) => {
    ...
    try {
      ...
      if(req.query.edt_name) {
  
        qry = qry + ` AND edt_name = '${req.query.edt_name}'`;
...
router.post('/', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { edt_name, user_id, sort_id, status_id } = req.body

    //Check if record exists.
    const checkExist = await EventDeliveryTypeService.findByName(edt_name);

    //If record exists, do not post.
    if(checkExist != '') {

      res.send(`Member already exists: ${edt_name}`)

    } else {

      const { rows } = await db.query(`INSERT INTO ${tableName} (edt_name, user_id, sort_id, status_id) VALUES ($1, $2, $3, $4) RETURNING *`, [edt_name, user_id, sort_id, status_id])
...
router.put('/:id', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params
    const { edt_name, user_id, sort_id, status_id } = req.body
    const { rows } = await db.query(`UPDATE ${tableName} SET edt_name = $2, edt_time_updated = now(), user_id = $3, sort_id = $4, status_id = $5 WHERE id = $1`, [id, edt_name, user_id, sort_id, status_id])
