...
const tableName = 'cxahub.t_event_type';
const viewName = 'cxahub.v_event_type';
const orderBy = 'ORDER BY sort_id ASC';

//Set services used by route.
const EventTypeService = require('../../../services/event/EventTypeService');
...
router.get('/', [auth, service], cache(cachMins), async (req, res) => {
    ...
    try {
      ...
      if(req.query.et_name) {
  
        qry = qry + ` AND et_name = '${req.query.et_name}'`;
...
router.post('/', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { et_name, user_id, sort_id, status_id } = req.body

    //Check if record exists.
    const checkExist = await EventTypeService.findByName(et_name);

    //If record exists, do not post.
    if(checkExist != '') {

      res.send(`Member already exists: ${et_name}`)

    } else {

      const { rows } = await db.query(`INSERT INTO ${tableName} (et_name, user_id, sort_id, status_id) VALUES ($1, $2, $3, $4) RETURNING *`, [et_name, user_id, sort_id, status_id])
...
router.put('/:id', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params
    const { et_name, user_id, sort_id, status_id } = req.body
    const { rows } = await db.query(`UPDATE ${tableName} SET et_name = $2, et_time_updated = now(), user_id = $3, sort_id = $4, status_id = $5 WHERE id = $1`, [id, et_name, user_id, sort_id, status_id])
