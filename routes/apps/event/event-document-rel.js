...
const tableName = "cxahub.t_event_document_rel";
const viewName = "cxahub.v_event_document_rel";
const orderBy = "";

//Set messaging.
const createSuccessMessage =
  "You have created the event document rel. successfully.";
const deleteSuccessMessage =
  "You have deleted the event document rel. successfully.";

//Set services used by route.
//const EventIndustryRelService = require('../../../services/event/EventIndustryRelService');
...
router.get("/", [auth, service], cache(cachMins), async (req, res) => {
  try {
    ...
    if (req.query.e_id && req.query.doc_id) {
      qry =
        qry +
        ` AND (e_id = '${req.query.e_id}' AND doc_id = '${req.query.doc_id}')`;
    }
...
router.post(
  ...
  wrap(async function (req, res) {
    const { e_id, doc_id } = req.body;

    //First delete any existing relationship records.
    await EventService.deleteEventDocumentRel(e_id);

    await db.query(
      `INSERT INTO ${tableName} (e_id, doc_id) VALUES ($1, $2) RETURNING *`,
      [e_id, doc_id]
    );
