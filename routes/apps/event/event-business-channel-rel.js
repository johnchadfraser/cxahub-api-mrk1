...
const tableName = 'cxahub.t_event_business_channel_rel';
const viewName = 'cxahub.v_event_business_channel_rel';
const orderBy = '';

//Set messaging.
const createSuccessMessage = 'You have created the event business channel rel. successfully.';
const deleteSuccessMessage = 'You have deleted the event business channel rel. successfully.';
...
router.get('/', [auth, service], cache(cachMins), async (req, res) => {
  ...
  try {
    ...
    if(req.query.e_id && req.query.bc_id) {

      qry = qry + ` AND (e_id = '${req.query.e_id}' AND bc_id = '${req.query.bc_id}')`;
...
router.post('/', [auth, service], wrap(async function(req, res) {
  ...
  const { e_id, bc_id } = req.body

  //First delete any existing relationship records.
  await EventService.deleteEventBusinessChannelRel(e_id);

  await db.query(`INSERT INTO ${tableName} (e_id, bc_id) VALUES ($1, $2) RETURNING *`, [e_id, bc_id])
