...
const tableName = "cxahub.t_event_industry_rel";
const viewName = "cxahub.v_event_industry_rel";
const orderBy = "";

//Set messaging.
const createSuccessMessage =
  "You have created the event industry rel. successfully.";
const deleteSuccessMessage =
  "You have deleted the event industry rel. successfully.";
...
router.get("/", [auth, service], cache(cachMins), async (req, res) => {
  try {
    ...
    if (req.query.e_id && req.query.i_id) {
      qry =
        qry +
        ` AND (e_id = '${req.query.e_id}' AND i_id = '${req.query.i_id}')`;
    }
...
router.post(
  ...
  wrap(async function (req, res) {
    const { e_id, i_id } = req.body;

    //First delete any existing relationship records.
    await EventService.deleteEventIndustryRel(e_id);

    await db.query(
      `INSERT INTO ${tableName} (e_id, i_id) VALUES ($1, $2) RETURNING *`,
      [e_id, i_id]
    );
