...
const tableName = 'cxahub.t_navigation_type';
const viewName = 'cxahub.v_navigation_type';
const orderBy = 'ORDER BY sort_id ASC';

//Set services used by route.
const NavigationTypeService = require('../../../services/navigation/NavigationTypeService');
...
router.get('/', [auth, service], cache(cachMins), async (req, res) => {
    ...
    try {
      ...
      if(req.query.navt_name) {
  
        qry = qry + ` AND navt_name = '${req.query.navt_name}'`;
...
router.post('/', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { navt_name, user_id, sort_id, status_id } = req.body

    //Check if record exists.
    const checkExist = await NavigationTypeService.findByName(navt_name, baseURL);

    //If record exists, do not post.
    if(checkExist != '') {

      res.send(`Member already exists: ${navt_name}`)

    } else {

      const { rows } = await db.query(`INSERT INTO ${tableName} (navt_name, user_id, sort_id, status_id) VALUES ($1, $2, $3, $4) RETURNING *`, [navt_name, user_id, sort_id, status_id])
...
router.put('/:id', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params
    const { navt_name, user_id, sort_id, status_id } = req.body
    const { rows } = await db.query(`UPDATE ${tableName} SET navt_name = $2, navt_time_updated = now(), user_id = $3, sort_id = $4, status_id = $5 WHERE id = $1`, [id, navt_name, user_id, sort_id, status_id])
