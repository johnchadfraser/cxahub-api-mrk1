...
const tableName = 'cxahub.t_user_type';
const viewName = 'cxahub.v_user_type';
const orderBy = 'ORDER BY sort_id ASC';

//Set services used by route.
const UserTypeService = require('../../../services/user/UserTypeService');
...
router.get('/', [auth, service], cache(cachMins), async (req, res) => {
    ...
    try {
      ...
      if(req.query.u_type_name) {
  
        qry = qry + ` AND u_type_name = '${req.query.u_type_name}'`;
...
router.post('/', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { u_type_name, user_id, sort_id, status_id } = req.body

    //Check if record exists.
    const checkExist = await UserTypeService.findByName(u_type_name);

    //If record exists, do not post.
    if(checkExist != '') {

      res.send(`Member already exists: ${u_type_name}`)

    } else {

      const { rows } = await db.query(`INSERT INTO ${tableName} ( u_type_name, user_id, sort_id, status_id) VALUES ($1, $2, $3, $4) RETURNING *`, [u_type_name, user_id, sort_id, status_id])
...
router.put('/:id', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params
    const { u_type_name, user_id, sort_id, status_id } = req.body
    const { rows } = await db.query(`UPDATE ${tableName} SET u_type_name = $2,u_type_time_updated = now(), user_id = $3, sort_id = $4, status_id = $5 WHERE id = $1`, [id, u_type_name, user_id, sort_id, status_id])
