...
const tableName = 'cxahub.t_user_field_type';
const viewName = 'cxahub.v_user_field_type';
const orderBy = 'ORDER BY sort_id ASC';

//Set services used by route.
const UserFieldTypeService = require('../../../services/user/UserFieldTypeService');
...
router.get('/', [auth, service], cache(cachMins), async (req, res) => {
    ...
    try {
      ...
      if(req.query.uft_name) {
  
        qry = qry + ` AND uft_name = '${req.query.uft_name}'`;
...
router.post('/', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { uft_name, user_id, sort_id, status_id } = req.body

    //Check if record exists.
    const checkExist = await UserFieldTypeService.findByName(uft_name);

    //If record exists, do not post.
    if(checkExist != '') {

      res.send(`Member already exists: ${uft_name}`)

    } else {

      const { rows } = await db.query(`INSERT INTO ${tableName} ( uft_name, user_id, sort_id, status_id) VALUES ($1, $2, $3, $4) RETURNING *`, [uft_name, user_id, sort_id, status_id])
...
router.put('/:id', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params
    const { uft_name, user_id, sort_id, status_id } = req.body
    const { rows } = await db.query(`UPDATE ${tableName} SET uft_name = $2, uft_time_updated = now(), user_id = $3, sort_id = $4, status_id = $5 WHERE id = $1`, [id, uft_name, user_id, sort_id, status_id])
