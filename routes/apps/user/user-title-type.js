...
const tableName = 'cxahub.t_user_title_type';
const viewName = 'cxahub.v_user_title_type';
const orderBy = 'ORDER BY sort_id ASC';

//Set services used by route.
const UserTitleTypeService = require('../../../services/user/UserTitleTypeService');
...
router.get('/', [auth, service], cache(cachMins), async (req, res) => {
    ...
    try {
      ...
      if(req.query.utt_name) {
  
        qry = qry + ` AND utt_name = '${req.query.utt_name}'`;
...
router.post('/', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { utt_name, user_id, sort_id, status_id } = req.body

    //Check if record exists.
    const checkExist = await UserTitleTypeService.findByName(utt_name);

    //If record exists, do not post.
    if(checkExist != '') {

      res.send(`Member already exists: ${utt_name}`)

    } else {

      const { rows } = await db.query(`INSERT INTO ${tableName} ( utt_name, user_id, sort_id, status_id) VALUES ($1, $2, $3, $4) RETURNING *`, [utt_name, user_id, sort_id, status_id])
...
router.put('/:id', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params
    const { utt_name, user_id, sort_id, status_id } = req.body
    const { rows } = await db.query(`UPDATE ${tableName} SET utt_name = $2,utt_time_updated = now(), user_id = $3, sort_id = $4, status_id = $5 WHERE id = $1`, [id, utt_name, user_id, sort_id, status_id])
