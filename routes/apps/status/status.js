...
const tableName = 'cxahub.t_status';
const viewName = 'cxahub.v_status';
...
router.get('/', [auth, service], cache(cachMins), async (req, res) => {
    ...
    try {
      ...
      if(req.query.status_name) {
  
        qry = qry + ` AND status_name = '${req.query.status_name}'`;
...
router.post('/', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { status_name, sort_id, status_id } = req.body

    //Check if record exists.
    const checkExist = await StatusService.findByName(status_name);

    //If record exists, do not post.
    if(checkExist != '') {

      res.send(`Member already exists: ${status_name}`)

    } else {

      const { rows } = await db.query(`INSERT INTO ${tableName} (status_name, sort_id, status_id) VALUES ($1, $2, $3) RETURNING *`, [status_name, sort_id, status_id])
...
router.put('/:id', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params
    const { status_name, sort_id, status_id } = req.body
    const { rows } = await db.query(`UPDATE ${tableName} SET status_name = $2, sort_id = $3, status_id = $4 WHERE id = $1`, [id, status_name, sort_id, status_id])
