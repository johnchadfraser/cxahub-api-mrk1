...
const tableName = 'cxahub.t_business_channel';
const viewName = 'cxahub.v_business_channel';
const orderBy = 'ORDER BY sort_id ASC';

//Set services used by route.
const BusinessChannelService = require('../../../services/businesschannel/BusinessChannelService');
...
router.get('/', [auth, service], cache(cachMins), async (req, res) => {
    ...
    try {
      ...
      if(req.query.bc_name) {
  
        qry = qry + ` AND bc_name = '${req.query.bc_name}'`;
...
router.post('/', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { bc_name, user_id, sort_id, status_id } = req.body

    //Check if record exists.
    const checkExist = await BusinessChannelService.findByName(bc_name);

    //If record exists, do not post.
    if(checkExist != '') {

      res.send(`Member already exists: ${bc_name}`)

    } else {

      const { rows } = await db.query(`INSERT INTO ${tableName} (bc_name, user_id, sort_id, status_id) VALUES ($1, $2, $3, $4) RETURNING *`, [bc_name, user_id, sort_id, status_id ])
...
router.put('/:id', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params
    const { bc_name, user_id, sort_id, status_id  } = req.body
    const { rows } = await db.query(`UPDATE ${tableName} SET bc_name = $2, bc_time_updated = now(), user_id = $3, sort_id = $4, status_id = $5 WHERE id = $1`, [id, bc_name, user_id, sort_id, status_id])
