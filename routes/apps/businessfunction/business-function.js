...
const tableName = "cxahub.t_business_function";
const viewName = "cxahub.v_business_function";
const orderBy = "ORDER BY sort_id, bf_name ASC";
...
router.get("/", [auth, service], cache(cachMins), async (req, res) => {
  ...
  if (req.query.bf_name) {
    qry = qry + ` AND bf_name = '${req.query.bf_name}'`;
  }
...
router.post("/", [auth, service], async (req, res) => {
  try {
    const { bf_name, user_id, sort_id, status_id } = req.body;

    //Check if record exists.
    const checkExist = await BusinessFunctionService.findByName(bf_name);

    //If record exists, do not post.
    if (checkExist != "") {
      res.send(`Member already exists: ${bf_name}`);
    } else {
      const { rows } = await db.query(
        `INSERT INTO ${tableName} (bf_name, user_id, sort_id, status_id) VALUES ($1, $2, $3, $4) RETURNING *`,
        [bf_name, user_id, sort_id, status_id]
      );
...
router.put("/:id", [auth, service], async (req, res) => {
  try {
    ...
    const { bf_name, user_id, sort_id, status_id } = req.body;
    const { rows } = await db.query(
      `UPDATE ${tableName} SET bf_name = $2, bc_time_updated = now(), user_id = $3, sort_id = $4, status_id = $5 WHERE id = $1`,
      [id, bf_name, user_id, sort_id, status_id]
    );
