...
const tableName = 'cxahub.t_application_type';
const viewName = 'cxahub.v_application_type';
const orderBy = 'ORDER BY sort_id ASC';

//Set services used by route.
const ApplicationTypeService = require('../../../services/application/ApplicationTypeService');
...
router.get('/', [auth, service], cache(cachMins), async (req, res) => {
    ...
    try {
      ...
      if(req.query.appt_name) {
  
        qry = qry + ` AND appt_name = '${req.query.appt_name}'`;
...
router.post('/', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { appt_name, user_id, sort_id, status_id } = req.body

    //Check if record exists.
    const checkExist = await ApplicationTypeService.findByName(appt_name, baseURL);

    //If record exists, do not post.
    if(checkExist != '') {

      res.send(`Member already exists: ${appt_name}`)

    } else {

      const { rows } = await db.query(`INSERT INTO ${tableName} (appt_name, user_id, sort_id, status_id) VALUES ($1, $2, $3, $4) RETURNING *`, [appt_name, user_id, sort_id, status_id])
...
router.put('/:id', [auth, service], async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params
    const { appt_name, user_id, sort_id, status_id } = req.body
    const { rows } = await db.query(`UPDATE ${tableName} SET appt_name = $2, appt_time_updated = now(), user_id = $3, sort_id = $4, status_id = $5 WHERE id = $1`, [id, appt_name, user_id, sort_id, status_id])
